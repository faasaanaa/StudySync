...
function ProfileCard({
  ...
  // Check if this profile is already bookmarked
  useEffect(() => {
    if (!user) return;
    const fetchBookmarks = async () => {
      ...
      if (snap.exists()) {
        ...
        setSaved(Array.isArray(data.bookmarked) && data.bookmarked.includes(profile.uid));
      }
    };
    fetchBookmarks();
  }, [user, profile.uid]);
  ...
  const handleBookmark = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (!snap.exists()) {
        await setDoc(userRef, { bookmarked: [profile.uid] }, { merge: true });
      } else {
        const data = snap.data();
        isBookmarked = Array.isArray(data.bookmarked) && data.bookmarked.includes(profile.uid);
      ...
      // Log activity for the other user (client-only demo)
      if (!isBookmarked) {
        // Add activity to the bookmarked user's localStorage
        const activityKey = `studysync_activities_${profile.uid}`;
        const activities = JSON.parse(localStorage.getItem(activityKey) || '[]');
        activities.unshift({
          type: 'bookmark',
          user: user.displayName || user.email || 'Someone',
          time: new Date().toISOString(),
          message: 'bookmarked your profile',
        });
        localStorage.setItem(activityKey, JSON.stringify(activities.slice(0, 20)));
      }
    } catch (err) {
