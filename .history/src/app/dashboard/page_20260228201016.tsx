...
const DEMO_ACTIVITIES = [
  ...
  },
  {
    ...
    message: 'bookmarked your profile',
  },
  {
    type: 'message',
    user: 'Fatima Khan',
  ...
  },
  {
    ...
    message: 'is a new match',
  },
  {
    type: 'leaderboard',
    user: 'You',
    time: new Date(Date.now() - 1000 * 60 * 60 * 24).toISOString(),
    message: 'moved up to #5 on the leaderboard',
  },
];

function getDemoActivities() {
  // For demo, store in localStorage so it persists for the session
  if (typeof window !== 'undefined') {
    const stored = localStorage.getItem('studysync_demo_activities');
    if (stored) return JSON.parse(stored);
    localStorage.setItem('studysync_demo_activities', JSON.stringify(DEMO_ACTIVITIES));
...
function ActivityFeed() {
  ...
  const iconFor = (type: string) => {
    switch (type) {
      case 'profile_visit': return <Users className="w-5 h-5 text-[#06B6D4]" />;
      case 'bookmark': return <Bookmark className="w-5 h-5 text-pink-400" />;
      case 'message': return <MessageSquare className="w-5 h-5 text-[#7C3AED]" />;
      case 'session_request': return <Calendar className="w-5 h-5 text-amber-400" />;
      case 'session_complete': return <CalendarCheck className="w-5 h-5 text-emerald-400" />;
      case 'review': return <Star className="w-5 h-5 text-yellow-400" />;
      case 'match': return <UserCheck className="w-5 h-5 text-[#7C3AED]" />;
      case 'leaderboard': return <TrendingUp className="w-5 h-5 text-[#06B6D4]" />;
