    ...
    <GlassCard hover={false} className="p-5">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex-1 min-w-0 space-y-2">
          ...
          {/* Show session ID if dateTime is missing */}
          {!session.dateTime && (
            <div className="text-xs text-cyan-400">Session ID: {session.id}</div>
          )}
  ...
  useEffect(() => {
    ...
    const fetchSessions = async () => {
      ...
      try {
        ...
        // Auto-prompt review modal for completed, both-confirmed, not-yet-reviewed sessions
        const reviewsSnapshot = await getDocs(
          query(collection(db, 'reviews'), where('reviewerId', '==', currentUid))
        );
        const reviewedSessionIds = new Set<string>();
        reviewsSnapshot.forEach((d) => {
          reviewedSessionIds.add(d.data().sessionId);
        });
        const needsReview = all.find(
          (s) =>
            s.status === 'completed' &&
            s.tutorConfirmed &&
            s.learnerConfirmed &&
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          <Link href="/sessions/join">
            <GlassButton variant="primary" size="md">Join Session</GlassButton>
          </Link>
          <Link href="/sessions/create">
            <GlassButton variant="secondary" size="md">Create Session</GlassButton>
          </Link>
        </div>

        {/* Chat & Notes Archive */}
        <div className="mb-10">
          <h2 className="text-xl font-bold text-white mb-2">Session Chat & Notes Archive</h2>
          <div className="space-y-2">
            {sessions.filter(s => s.status === 'completed').length === 0 ? (
              <div className="text-white/40 text-sm">No completed sessions yet.</div>
            ) : (
              sessions.filter(s => s.status === 'completed').map(s => (
                <GlassCard key={s.id} className="p-4 flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-white">{s.subject}</span>
                    <span className="text-xs text-white/40">{new Date(s.dateTime).toLocaleString()}</span>
                  </div>
                  <div className="flex gap-2 mt-2">
                    <Link href={`/room/${s.id}`}>
                      <GlassButton variant="ghost" size="sm">View Chat & Notes</GlassButton>
                    </Link>
                  </div>
                </GlassCard>
              ))
