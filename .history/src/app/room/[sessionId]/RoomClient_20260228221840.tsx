...
function StudyRoomPage() {
  ...
  useEffect(() => {
    async function fetchSession() {
      ...
      if (snap.exists()) {
        ...
        setSessionName(data.sessionName || data.subject || "Session");
  ...
  const handleEndSession = async () => {
    ...
    try {
      const sessionRef = doc(db, 'sessions', String(sessionId));
      await import('firebase/firestore').then(async ({ updateDoc, increment }) => {
        await updateDoc(sessionRef, { status: 'completed' });
        // Optionally update user stats here if you have user context
        // Example: await updateDoc(doc(db, 'users', userId), { totalSessions: increment(1) });
      });
    ...
    <div className="min-h-screen bg-[#0A0A0F] text-white">
      ...
      >
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          {/* Focus Mode & End Session Buttons */}
          {sessionStatus === 'ongoing' && (
            <div className="flex gap-2">
              <GlassButton variant="secondary" size="sm" onClick={handleFocusMode}>
                Focus Mode
              </GlassButton>
              <GlassButton variant="danger" size="sm" onClick={handleEndSession} disabled={ending}>
