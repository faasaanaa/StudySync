...
export default function StudyRoomPage() {
  ...
  const handleEndSession = async () => {
    if (!sessionId || !user) {
      toast.error('Session or user missing.');
      return;
    }
    try {
      ...
      // Update user stats (totalSessions)
      const userRef = doc(db, 'users', user.uid);
      await updateDoc(userRef, { totalSessions: increment(1) });

      toast.success('Session ended. Stats updated!');
