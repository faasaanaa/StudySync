...
function getInitialChat(uid: string, displayName: string): ChatMessage[] {
  return [
    {
      id: 'msg-1',
      senderId: 'user-2',
      senderName: 'Sarah Ahmed',
      text: 'Hey! Ready to start? Let\'s tackle integration by parts today.',
    ...
    },
    {
      ...
      text: 'Sounds good! I have a few practice problems ready.',
...
function ChatNotesPanel() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => getInitialChat(currentUid, currentName));
  ...
  const handleNotesChange = (val: string) => {
    if (val.length > MAX_NOTES_CHARS) return;
    setNotes(val);
    setNotesSaved(false);
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      const replies = [
        ...
        'I agree, let\'s move on to the next problem.',
        'Can you explain that part again?',
        'Nice! I think we\'re making great progress.',
        ...
        {activeSubTab === 'chat' ? (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 rounded-xl bg-white/[0.06] border border-white/[0.12] text-white placeholder-white/30 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/50"
              />
              <GlassButton
                variant="primary"
                size="sm"
          ...
          >
            <textarea
              ...
              placeholder="Write your session notes here... They auto-save as you type."
              className="flex-1 w-full rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-white/30 p-4 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/50 resize-none custom-scrollbar"
            />
            <div className="flex items-center justify-between mt-2 text-xs text-white/40">
              <span className="flex items-center gap-1">
                {notesSaved ? (
                  ...
                  <>
                    <Clock size={12} className="text-yellow-400 animate-pulse" /> Saving...
                  </>
                )}
              </span>
              <span
                className={
                  notes.length > MAX_NOTES_CHARS * 0.9 ? 'text-yellow-400' : ''
                }
              >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/50">
                with <span className="text-cyan-400">{SESSION_INFO.partnerName}</span>
              </p>
            </div>
          </div>
